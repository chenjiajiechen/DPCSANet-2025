...
def customBlur(imgFile):
    size = 512
    targetPath = r'/home/chenjiajie/桌面/Remo_detection/datasets/OSSD/train/degrade'  # save path
    labelFile = imgFile.replace('jpg', 'txt').replace('images', 'labels')
...
if __name__=='__main__':
    sourcePath = r'/home/chenjiajie/桌面/Remo_detection/datasets/OSSD/train/images'  # source path
    with concurrent.futures.ProcessPoolExecutor(1) as executor:  # set 1 to other number for speedup
        imgfiles=glob(os.path.join(sourcePath, '*.jpg'))
