...
#              alpha=1, gamma=0.5, scale=False, eps=1e-9):
...
#     else:
#         return iou  # IoU
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU_05=False, CIoU=False, CIoU_k=False, EIoU=False, FIoU=False, eps=1e-7, alpha=1, gamma=0.5, Focal=False):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    ...
    # 目标框IOU损失函数的计算
    if CIoU or DIoU or GIoU or EIoU or FIoU or CIoU_05 or CIoU_k:
        # 两个框的最小闭包区域的width
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        # 两个框的最小闭包区域的height
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        c_area = cw * ch + eps  # convex area
        if CIoU or DIoU or EIoU or FIoU or CIoU_05 or CIoU_k:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            # 最小外接矩形 对角线的长度平方
            c2 = cw ** 2 + ch ** 2 + eps  # convex diagonal squared
            # 两个框中心点之间距离的平方
            rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2) ** 2 +
            ...
            # CIoU 比DIoU多了限制长宽比的因素：v * alpha
                ...
                v = (4 / math.pi ** 2) * torch.pow(torch.atan(w2 / h2) - torch.atan(w1 / h1), 2)  # 宽高比
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))  # 对形状loss加权，距离远的，影响越小
                ...
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))  # 对形状loss加权，距离远的，影响越小
                    # alpha = 2 * iou + eps / (1 + iou)  # 对形状loss加权，距离远的，影响越小
                # return torch.pow(iou, 2) - (torch.pow(rho2 / c2, 2) + torch.pow(v * alpha, 2)), torch.pow(1 - iou + eps, gamma)
                # return iou - (rho2 / c2 + v * alpha), torch.pow(1 - iou + eps, gamma)
                return iou - (rho2 / c2 + v * alpha), torch.pow(1.5 - iou, gamma)
                # return iou - (torch.pow(rho2 / c2, 0.5) + v * alpha + rho_w2 / cw2)
                # return rho2 / c2 + v * alpha + rho_w2 / cw2 + eps
            # EIoU 在CIoU的基础上将纵横比的损失项拆分成预测的宽高分别与最小外接框宽高的差值 加速了收敛提高了回归精度
                ...
                ch2 = ch ** 2 + eps
                if Focal:
                    return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2), torch.pow(inter / union, gamma)  # Focal_EIou
                else:
                    return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2)
            elif FIoU:
                k = 1 / (2 - iou)  # 对形状loss加权，距离远的，影响越小
                rho_w2 = ((b2_x2 - b2_x1) - (b1_x2 - b1_x1)) ** 2
                rho_h2 = ((b2_y2 - b2_y1) - (b1_y2 - b1_y1)) ** 2
                cw2 = cw ** 2 + eps
                ch2 = ch ** 2 + eps
                # return torch.pow(iou, 3) - (torch.pow(rho2 / c2, 3) + torch.pow(k * (rho_w2 / cw2 + rho_h2 / ch2), 3))
                return iou - (rho2 / c2 + k*(rho_w2 / cw2 + rho_h2 / ch2))
            elif CIoU_k:
                v = (4 / math.pi ** 2) * torch.pow(torch.atan(w2 / h2) - torch.atan(w1 / h1), 2)  # 宽高比
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))  # 对形状loss加权，距离远的，影响越小
                return iou - (rho2 / c2 + v * alpha), torch.pow(inter / union, gamma)
        else:
